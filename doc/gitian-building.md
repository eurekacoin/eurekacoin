...
*Setup instructions for a Gitian build of EurekaCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EurekaCoin
...
- [Building EurekaCoin Core](#building-eurekacoin-core)
...
Clone the git repositories for eurekacoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/eurekacoin/eurekacoin --recursive
git clone https://github.com/eurekacoin/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/eurekacoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the eurekacoin repository under 'Fetch and create inputs' to install sources which require
...
To build EurekaCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the eurekacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/eurekacoin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/eurekacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit eurekacoin=${COMMIT} --url eurekacoin=${URL} ../eurekacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit eurekacoin=${COMMIT} --url eurekacoin=${URL} ../eurekacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit eurekacoin=${COMMIT} --url eurekacoin=${URL} ../eurekacoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-eurekacoin:
./bin/gbuild --commit eurekacoin=${COMMIT},cpp-eth-eurekacoin=${ETHCOMMIT} --url eurekacoin=${URL},cpp-eth-eurekacoin=${ETHURL} ../eurekacoin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the eurekacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../eurekacoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/eurekacoin/eurekacoin-detached-sigs.git

BTCPATH=/some/root/path/eurekacoin
SIGPATH=/some/root/path/eurekacoin-detached-sigs

./bin/gbuild --url eurekacoin=${BTCPATH},signature=${SIGPATH} ../eurekacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/eurekacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/eurekacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/eurekacoin-osx-build.assert
```
...
[eurekacoin/gitian.sigs](https://github.com/eurekacoin/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@eurekacoin.org) and he will commit them.