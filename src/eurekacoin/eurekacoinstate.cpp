...
#include <eurekacoin/eurekacoinstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

EurekaCoinState::EurekaCoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = EurekaCoinState::openDB(_path + "/eurekacoinDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

EurekaCoinState::EurekaCoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute EurekaCoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, EurekaCoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createEurekaCoinAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            eurekacoin::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> EurekaCoinState::vins() const // temp
{
...
void EurekaCoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* EurekaCoinState::vin(dev::Address const& _a) const
{
    return const_cast<EurekaCoinState*>(this)->vin(_a);
}

Vin* EurekaCoinState::vin(dev::Address const& _addr)
{
...
// void EurekaCoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void EurekaCoinState::kill(dev::Address _addr)
{
...
void EurekaCoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address EurekaCoinState::createEurekaCoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void EurekaCoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void EurekaCoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void EurekaCoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
