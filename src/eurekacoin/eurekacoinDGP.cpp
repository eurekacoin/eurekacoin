#include <eurekacoin/eurekacoinDGP.h>

void EurekaCoinDGP::initDataEIP158(){
...
bool EurekaCoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule EurekaCoinDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t EurekaCoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t EurekaCoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t EurekaCoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t EurekaCoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool EurekaCoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void EurekaCoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void EurekaCoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void EurekaCoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void EurekaCoinDGP::createParamsInstance(){
...
dev::Address EurekaCoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void EurekaCoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void EurekaCoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void EurekaCoinDGP::parseStorageOneUint64(uint64_t& value){
...
void EurekaCoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule EurekaCoinDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
