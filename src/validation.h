...
#include <eurekacoin/storageresults.h>
...
using ExtractEurekaCoinTX = std::pair<std::vector<EurekaCoinTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE = 0.25 * COIN;
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // eurekacoin
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashEurekaCoinRoot,
    const dev::h256& oldHashStateRoot, const std::vector<EurekaCoinTransaction>& transactions);
...
class EurekaCoinTxConverter{

public:

    EurekaCoinTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionEurekaCoinTransactions(ExtractEurekaCoinTX& eurekacoinTx);
...
private:
    ...
    EurekaCoinTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<EurekaCoinTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
...
private:
    ...
    std::vector<EurekaCoinTransaction> txs;
