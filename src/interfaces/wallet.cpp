...
public:
    ...
    bool isUnspentAddress(const std::string &eurekacoinAddress) override
    {
        LOCK2(::cs_main, m_wallet.cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == eurekacoinAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
