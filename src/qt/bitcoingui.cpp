...
#include <qt/eurekacoinversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    eurekacoinVersionChecker = new EurekaCoinVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a EurekaCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and eurekacoin: URIs)"));
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken"), tr("&ERC Tokens"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your EurekaCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified EurekaCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a eurekacoin: URI or payment request"));//EUREKACOIN_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible EurekaCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && eurekacoinVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(EUREKACOIN_RELEASES, EUREKACOIN_RELEASES);
            QString message(tr("New version of EurekaCoin wallet is available on the EurekaCoin source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to EurekaCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("EurekaCoin"); // default title
