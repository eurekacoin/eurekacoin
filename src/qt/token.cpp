...
bool ToHash160(const std::string& strEurekaCoinAddress, std::string& strHash160)
{
    CTxDestination eurekacoinAddress = DecodeDestination(strEurekaCoinAddress);
    if(!IsValidDestination(eurekacoinAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&eurekacoinAddress);
    if(keyid){
...
bool ToEurekaCoinAddress(const std::string& strHash160, std::string& strEurekaCoinAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination eurekacoinAddress = keyid;
    if(IsValidDestination(eurekacoinAddress)){
        strEurekaCoinAddress = EncodeDestination(eurekacoinAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToEurekaCoinAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToEurekaCoinAddress(tokenEvent.receiver, tokenEvent.receiver);
