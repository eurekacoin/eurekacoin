...
#define paternVersion "eurekacoin-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

EurekaCoinVersionChecker::EurekaCoinVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

EurekaCoinVersionChecker::~EurekaCoinVersionChecker()
{

}

bool EurekaCoinVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> EurekaCoinVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(EUREKACOIN_RELEASES)));
    QEventLoop event;
...
Version EurekaCoinVersionChecker::getMaxReleaseVersion()
{
