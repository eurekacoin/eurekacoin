...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setEurekaCoinSchedule(eurekacoinDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = eurekacoinDGP.getBlockSize(nHeight);
    minGasPrice = eurekacoinDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = eurekacoinDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    EurekaCoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractEurekaCoinTX resultConverter;
    if(!convert.extractionEurekaCoinTransactions(resultConverter)){
    ...
    for(EurekaCoinTransaction eurekacoinTransaction : eurekacoinTransactions){
        txGas += eurekacoinTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + eurekacoinTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(eurekacoinTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, eurekacoinTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "eurekacoinstake";
...
void StakeEurekaCoins(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
